...
`chain_region_brief`의 **연설용 한 줄 모드**가 핵심 지표를 자동으로 추려 한 문장으로 만듭니다. 요청 지역 직접값만 가용 지표로 집계하며, 상위 시도·전국 참고값은 별도 `references`로 분리합니다.
...
| | `quick_rank` 🆕 | "우리 지역 전국 몇 위?" — 17개 시도 또는 같은 행정유형(시·군·자치구·일반구) 내 순위·백분위·평균 격차·순위 변동 |
| **출처·각주** 🆕 | `explain_statistic` | 통계 공식 정의·작성목적·조사주기·용어해설 + 보고서 인용 각주. `quick_stats.source` 전체 전달 시 통계표·항목 ID 일치 보장 |
| **체인** ⛓ | `chain_region_brief` | 한 지역 13개 지표 종합 브리핑. 직접값과 상위지역 참고값을 분리 (연설용 한 줄 모드 포함) |
| | `chain_compare_regions` | N개 지역 × M개 지표 매트릭스 + 순위. 상위지역 참고값은 순위 제외 (최대 17×8) |
